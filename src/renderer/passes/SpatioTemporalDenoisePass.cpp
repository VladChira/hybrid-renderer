...
namespace hybrid::renderer
{
    ...
    namespace
    {
        ...
        constexpr GLuint kTemporalMomentsImageBinding = 1;
                                            ...
                                            SpatioTemporalDenoisePassOutput &output)
    {
        ...
        // Without a previous-frame gbuffer we cannot do disocclusion checks, so
        // fall back to "always reject" by routing the current gbuffer in and
        // disabling history.
        const bool history_valid = input.history_valid && prev_gbuffer_available;
        ...
        // ----------------------------
        {
            ...
            m_temporal_program->SetUniform1ui("u_history_valid",        history_valid ? 1u : 0u);
            ...
            // history (m1, m2) carried across frames
            glBindImageTexture(kTemporalHistoryImageBinding,
                               ...
                               GL_WRITE_ONLY,
                               GL_RG16F);
            // (m1, variance) consumed by this frame's a-trous
            glBindImageTexture(kTemporalMomentsImageBinding,
            ...
            for (GLuint unit = kTemporalCurrentTexUnit; unit <= kTemporalNormalPrevTexUnit; ++unit)
            {
                glActiveTexture(GL_TEXTURE0 + unit);
                if (unit == kTemporalCurrentTexUnit || unit == kTemporalHistoryTexUnit)
                {
        ...
        // The temporal pass left (m1, var) in atrous_ping. A-trous starts there.
        GlTextureId atrous_source = input.atrous_ping_array;
        GlTextureId atrous_target = input.atrous_pong_array;
        ...
        if (iterations > 0u)
        {
            ...
            m_atrous_program->SetUniform1f("u_sigma_l",     input.sigma_l);
            m_atrous_program->SetUniform1f("u_n_phi",       input.n_phi);
            m_atrous_program->SetUniform1f("u_p_phi",       input.p_phi);
            m_atrous_program->SetUniform1f("u_camera_near", input.camera_near);
            m_atrous_program->SetUniform1f("u_camera_far",  input.camera_far);
