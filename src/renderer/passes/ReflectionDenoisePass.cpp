...
namespace hybrid::renderer
{
                                        ...
                                        ReflectionDenoisePassOutput &output)
    {
...

        {
            ...
            glBindTexture(GL_TEXTURE_2D, input.current_signal);
            ...
            m_temporal_program->SetUniform1i("u_signal_current", static_cast<GLint>(kTemporalCurrentTexUnit));
            ...
            if (output_size_loc >= 0)
            {
                glUniform2ui(output_size_loc,
                             static_cast<GLuint>(input.extent.width),
                             static_cast<GLuint>(input.extent.height));
            }
        ...
        if (input.atrous_iterations > 0u)
        {
            ...
            for (uint32_t iteration = 0; iteration < input.atrous_iterations; ++iteration)
            {
                m_atrous_program->SetUniform1i("u_stepwidth", 1 << iteration);
        ...
        output.history = input.history_out;
