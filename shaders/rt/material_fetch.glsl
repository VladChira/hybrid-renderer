...
// (sampler2D(uvec2) construction is an extension feature).
...
struct ShadedSurface
{
    ...
    float roughness;
    vec3  emissive;
    vec3  normal_tangent;  // tangent-space normal sampled from the normal map
};
...
ShadedSurface FetchSurface(uint material_index, vec2 uv0, vec2 uv1)
{
    ...
    surface.normal_tangent = normalize(tangent_normal);
    return surface;
}

// Alpha-cutoff gate — callers that care about mask materials can use this to
// skip a hit on transparent pixels.
...
#endif // HYBRID_RT_MATERIAL_FETCH_GLSL
