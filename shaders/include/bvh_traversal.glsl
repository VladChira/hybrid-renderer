...
bool TraceClosestHit(vec3 origin, vec3 direction, uint tlas_node_count, float t_max, out RayHit hit)
{
    ...
    int root_index = 0;
    BvhNode root_node = tlas_nodes[root_index];
    float root_near_t = IntersectAabbNearT(origin, tlas_inv_dir, root_node.bmin, root_node.bmax, 0.0, best_t);
    if (root_near_t >= best_t)
    {
        return false;
    }
    ...
    vec3 blas_inv_dir = vec3(0.0);
    GpuPrimitive blas_primitive;
    uint blas_tlas_instance_idx = 0u;
    ...
    while (true)
    {
        if (has_pending_instances)
        {
            ...
            if (!top_is_blas)
            {
                ...
                while (pending_next < pending_count)
                {
                    uint inst_idx = uint(pending_first + pending_next);
                    GpuTlasInstance inst = tlas_instances[inst_idx];
                    GpuPrimitive prim = primitives[inst.primitive_id];
                    if (prim.index_count == 0u)
                    {
                        pending_next += 1;
                    ...
                    blas_inv_dir = local_inv_dir;
                    blas_primitive = prim;
                    blas_tlas_instance_idx = inst_idx;
        ...
        while (true)
        {
            if (node.right_or_count < 0)
            {
                ...
                if (in_blas)
                {
                    for (int i = 0; i < count; ++i)
                    {
                        ...
                        vec3 v2 = vertices[blas_primitive.vertex_offset + i2].position;
                        float t, u, v;
                        if (IntersectTriangle(blas_origin, blas_direction, v0, v1, v2, 0.0, best_t, t, u, v))
                        {
                            best_t = t;
                            best_hit.tlas_instance_index = blas_tlas_instance_idx;
                            best_hit.triangle_in_blas = uint(first + i);
                            best_hit.t = t;
                            best_hit.bary = vec2(u, v);
                            found = true;
                        }
                    }
                }
            ...
            vec3 ray_origin_bvh = in_blas ? blas_origin : origin;
            vec3 ray_inv_dir_bvh = in_blas ? blas_inv_dir : tlas_inv_dir;

            float left_near_t  = IntersectAabbNearT(ray_origin_bvh, ray_inv_dir_bvh, left_node.bmin,  left_node.bmax,  0.0, best_t);
            float right_near_t = IntersectAabbNearT(ray_origin_bvh, ray_inv_dir_bvh, right_node.bmin, right_node.bmax, 0.0, best_t);
            bool left_hit  = left_near_t  < best_t;
            bool right_hit = right_near_t < best_t;
